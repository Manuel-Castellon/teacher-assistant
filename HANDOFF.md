...
- DB migration was not applied from this shell because `psql` is not installed and no shell `DATABASE_URL` was safely detected.
...
- Try real model generation with worksheet toggle on/off and compare the generated worksheet/key quality to `data/lesson-plans/generated/*-worksheet*`.
- Then try the signed-in `/curriculum` -> `/lesson-plan` -> post-lesson update -> `/exam` loop with real class data after applying the DB migration on a machine with `psql` and `DATABASE_URL`.
